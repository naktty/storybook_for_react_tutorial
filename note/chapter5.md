...
## コンポーネント駆動開発
Taskから始まり、TaskListへと進み、そして今、画面全体のUIがここにあります。私たちのInboxScreenは、接続されたコンポーネントを収容し、付随するストーリーを含んでいます。

コンポーネント駆動開発では、コンポーネント階層が上がるにつれて、徐々に複雑さを拡大していくことができます。その利点として、開発プロセスがより集中的になり、UIのあらゆる可能性をカバーできるようになります。つまり、CDDはより高品質で複雑なユーザーインターフェイスを構築するのに役立ちます。

私たちはまだ終わっていません - UIが構築されたら仕事は終わりではありません。UIが構築されても仕事は終わりではありません。
